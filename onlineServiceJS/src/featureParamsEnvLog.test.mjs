...
test('buildFeatureParamsEnvLogRecord 默认不脱敏，含 event/env/redact=false', () => {
...
test('isFeatureParamsEnvLogRedactEnabled 识别 FEATURE_PARAMS_ENV_LOG_REDACT 与 ENV_LOG_REDACT', () => {
  assert.equal(isFeatureParamsEnvLogRedactEnabled('1', ''), true);
  assert.equal(isFeatureParamsEnvLogRedactEnabled('', 'true'), true);
  assert.equal(isFeatureParamsEnvLogRedactEnabled('', ''), false);
