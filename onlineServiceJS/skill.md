...
- 容器引导多仓克隆（`task_api_bootstrap`）逻辑在 `src/bootstrap.mjs`；持有 PEM 时使用临时密钥与 `GIT_SSH_COMMAND`（细节以代码为准）。**nested 子仓**（`git_repo_entries[].parent_repo_url`）先克隆到 `.bootstrap-staging/`，成功后再移入 `{父仓目录}/{clone_alias path}`。克隆成功后按 task-detail 的 `target_branch` / `repo_branch_plans` 将各仓切换到工作分支（`bootstrapWorkBranch.mjs`）。
